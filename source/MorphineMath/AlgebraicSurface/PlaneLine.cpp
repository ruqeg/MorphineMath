...
/******************************************
 * гл. II | §3. Основные задачи о прямых и плоскостях
 ******************************************
 *	Определения.
 *		Определение 1.
 *			
 *	Предложения.
 *		Предложение 1.
 *			Уравнение прямой, проходящей через две точки можно задать формулой
 *				| x - x1    y - y1 |
 *				| x2 - x1  y2 - y1 | = 0
 *		Предложение 2.
 *			Уравнение плоскости, проходящей через три точки можно задать формулой
 *				| x - x1    y - y0   z - z1 |
 *				| x2 - x1  y2 - y1  z2 - z1 | = 0
 *				| x3 - x1  y3 - y1  z3 - z1 |
 *		Предложение 3.
 *			Критерий параллельности прямой и плоскости можно задать в виде
 *				| A   B   C  |
 *				| A1  B1  C1 | = 0	(4)
 *				| A2  B2  C2 |
 *		Предложение 4.
 *			Расстояние от точки до плоскости можно найти формулой
 *				h = |(R - r0, n)| / |n|
 *		Предложение 5.
 *			Расстояние от точки до прямой можно найти формулой
 *				h = |[R − r0, a]| / |a|
 *		Предложение 6.
 *			Расстояние между скрещивающимися прямыми можно найти формулой
 *				h = |(r2 − r1, a1, a2)| / |[a1, a2]|
 *	Теоремы.
 *		Теорема 1.
 *	Следствия.
 ...
 *			Пусть в пространстве задана общая декартова система координат и две точки M1 и M2 с координатами (x1, y1, z1) и (x2, y2, z2). 
 *			Чтобы написать уравнение прямой M1M2 примем M1 за начальную точку, а M1M2 за направляющий вектор.
 *			Этот вектор не нулевой, если точки не совпадают. По формуле (29) §2 мы получаем
 ...
 *			В планиметрии задача решается так же. Отличие только в том, что координаты точек теперь (x1, y1) и (x2, y2), и мы получаем по формуле (8) § 2
 *				| x - x1    y - y1 |
 *				| x2 - x1  y2 - y1 | = 0
 *			Предложение 1 доказано.
 *		Доказательство предложения 2.
 *			Пусть M1, M2 и M3 — не лежащие на одной прямой точки с координатами (x1, y1, z1), (x2, y2, z2) и (x3, y3, z3) в общей декартовой системе координат. 
 *			Выберем M1 в качестве начальной точки, а M1M2 и M1M3 в качестве направляющих векторов. 
 *			Тогда по формулам (13) § 2 и (12) § 5 гл. I получаем уравнение плоскости
 *				| x - x1    y - y0   z - z1 |
 *				| x2 - x1  y2 - y1  z2 - z1 | = 0
 *				| x3 - x1  y3 - y1  z3 - z1 |
 *			Предложение 2 доказано.
 *		Доказательство предложения 3.
 *			Пусть известен направляющий вектор прямой a(α1, α2, α3), а плоскость задана одним из уравнений (r − r0, n) = 0 или (r − r0, p, q) = 0.
 *			Прямая параллельна плоскости (а возможно, и лежит в ней) тогда и только тогда, когда соответственно (a, n) = 0 или (a, p, q) = 0. 
 *			Если плоскость задана линейным уравнением Ax + By + Cz + D = 0, то по предложению 5 §2 условие параллельности
 *				Aα1 + Bα2 + Cα3 = 0. (3)
 *			Пусть прямая задана системой уравнений
 *				A1x + B1y + C1z + D1 = 0, A2x + B2y + C2z + D2 = 0.
 *			Тогда по предложению 10 § 2 условие (3) переписывается в виде
 *				 | B    C |    | C    A |    | A    B |
 *				A| B1  C1 | + B| C1  A1 | + C| A1  B1 | = 0	(3)
 *			или
 *				| A   B   C  |
 *				| A1  B1  C1 | = 0	(4)
 *				| A2  B2  C2 |
 *			Легко проверить, что все приведенные здесь условия являются не только необходимыми, но и достаточными. 
 *			В частности, из формулы (4) следует, что три плоскости пересекаются в одной точке тогда и только тогда, когда коэффициенты их уравнений удовлетворяют условию
 *				| A   B   C  |
 *				| A1  B1  C1 | = ≠	(5)
 *				| A2  B2  C2 |
 *			Действительно, это неравенство означает, что прямая, по которой пересекаются две плоскости, не параллельна третьей.
 *			Нужно сказать, что это условие было нами по существу получено в алгебраической форме в предложении 8 § 5 гл. I.
 *			Сейчас мы снова пришли к нему исходя из другой геометрической интерпретации системы линейных уравнений.
 *			Предложение 3 доказано.
 *		Доказательство предложения 4.
 *			Пусть дана плоскость с уравнением (r − r0, n) = 0 и точка M с радиус вектором R. 
 *			Рассмотрим вектор M0M = R − r0, соединяющий начальную точку плоскости с M. 
 *			Расстояние от точки до плоскости равно модулю его скалярной проекции на вектор n, то есть
 *				h = |(R − r0, n)| / |n|	(6)
 *			Если в декартовой прямоугольной системе координат точка M имеет координаты (X, Y, Z), то равенство (8) запишется согласно предложениям 3 и 4 §2 так
 *				h = |AX + BY + CZ + D| / sqrt(A^2 + B^2 + C^2)	(7)
 *			Предложение 4 доказано.
 *		Доказательство предложения 5.
 *			Если прямая задана уравнением [r − r0, a] = 0, то мы можем найти расстояние h от точки M с радиус-вектором R до этой прямой, разделив
 *			площадь параллелограмма, построенного на векторах R − r0 и a на длину его основания. Результат можно записать формулой
 *				h = |[R − r0, a]| / |a|	(8)
 *			Для прямой в пространстве мы не будем получать координатной записи этого выражения.
 *			Рассмотрим прямую на плоскости, заданную уравнением Ax + By + C = 0 в декартовой прямоугольной системе координат.
 *			Пусть M0(x0, y0) — начальная точка прямой, а M(X, Y ) — некоторая точка плоскости. В качестве направляющего вектора возьмем вектор a(−B, A). 
 *			И з формулы (24) §5 гл. I следует, что площадь параллелограмма равна
 *				S = |(X − x0)A − (Y − y0)(−B)|.
 *			Но 
 *				−Ax0 − By0 = C и S = |AX + BY + C|. 
 *			Следовательно,
 *				h = |AX + BY + C| / sqrt(A^2 + B^2)	(9)
 *			Легко заметить также, что для нахождения расстояния от точки до прямой на плоскости можно воспользоваться рассуждением, примененным при выводе формулы (6). 
 *			Полученное нами равенство (9) — координатная запись формулы (6) в декартовой прямоугольной системе координат, если n(A, B) — нормальный вектор прямой с координатами (A, B).
 *			Предложение 5 доказано.
 *		Доказательство предложения 6.
 *			Пусть прямые p и q не параллельны. Известно, что в этом случае существуют такие параллельные плоскости P и Q, что прямая p лежит в P, а прямая q лежит в Q. (Если уравнения прямых r = r1 + a1t и r = r2 + a2t, то плоскость P имеет начальную точку r1 и направляющие векторы a1 и a2. Аналогично строится плоскость Q.)
 *			Расстояние h между P и Q называется расстоянием между прямыми p и q. Если p и q пересекаются, то P и Q совпадают и h = 0.
 *			Для того, чтобы найти расстояние h, проще всего разделить объем параллелепипеда, построенного на векторах r2 - r1, a1 и a2, на площадь его основания (рис. 23). Мы получим
 *				h = |(r2 − r1, a1, a2)| / |[a1, a2]|
